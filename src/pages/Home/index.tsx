  ...
  return (
    <View className={s.container}>
      <View className={s.title}>
        <View className={s.slogen}>Hi,&nbsp;&nbsp;{geneWelcomeMessage()}</View>
        <View className={s.user}>
          <AtAvatar
            circle
            image={
              userInfo?.avatarUrl ||
              'https://pic4.zhimg.com/80/v2-7f03cd2301fe14c49f8a0bf946b2de9d_1440w.jpg?source=1940ef5c'
            }
          />
          {loginInfo ? (
            <View className={s.detail}>
              ...
              <View className={s.desc}>
                ...
                <View className={s.message}>
                  {loginInfo?.sign || '今天也要加油哦今天是个好天气呢'}
                </View>
              </View>
            </View>
          ) : (
      ...
      <View className={s.calendar}>
        ...
        >
          <View className={s.pointer} onClick={() => toggleShowCalendarMask()}>
            {showCalendarMask ? (
              <Icon value="down" color={tabBlurColor} />
            ) : (
